...
namespace Balaso
{
    ...
    {
        private static string SETTINGS_ASSET_PATH = "Assets/_SDK/Scripts/Utility/Balaso Software/Editor/Settings.asset";
        ...
        public static Settings GetInstances()
        {
            ...
            Settings[] a = new Settings[guids.Length];
            for (int i = 0; i < guids.Length; i++)
            {
                string path = AssetDatabase.GUIDToAssetPath(guids[i]);
                a[i] = AssetDatabase.LoadAssetAtPath<Settings>(path);
            }
