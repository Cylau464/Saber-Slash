...
namespace apps
{
    public static class ADSManager
    {
        ...
        public static bool IsRewardedLoaded => s_ADSMaker.IsRewardedideoAvailable();
        public static bool IsInterstitialLoaded => s_ADSMaker.IsInterstitialAvailable();

        public static Action<bool> OnBannerSwitched;
        ...
        public static bool AutoShowInterstitial(string placementName, float timerToShow = -1)
        {
            if (placementName == null || placementName.CompareTo("") == 0)
                throw new ArgumentEmptryOrNullException();

            float showInterstitialEvery = (timerToShow <= 0) ? s_ShowInterstitialEvery : timerToShow;

            if (s_AllowAutoInterstitial == false)
                return false;

            if (s_LastTimerAds + showInterstitialEvery <= Time.time)
            {
