Shader "Drawing/MaskedPaint"
{
	Properties
	{
		...
		_BrushPos("Mask Position", Vector) = (0.5,0.5,0.0,0.0)
		_BrushSize("Mask Size", Range(0, 10)) = 1.0
		_BrushTexRotation("Rotation", Range(0, 360)) = 0
	...
	SubShader
	{
		...
		//	sampler2D _Mask;
			...
			fixed2 posWithRotation(fixed2 pos) {
				fixed rad = radians(_BrushTexRotation);
				fixed _s = sin(rad);
				fixed _c = cos(rad);
