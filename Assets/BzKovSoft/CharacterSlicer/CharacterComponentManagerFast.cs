...
namespace BzKovSoft.CharacterSlicer
{
	class CharacterComponentManagerFast : StaticComponentManager, IComponentManager
	{
		...
		private static void OnCompletePerSide(List<Collider> colliders, GameObject go)
		{
			...
			if (oldRoot == null)
			{
				var animator = go.GetComponentInChildren<Animator>(); //GetComponent<Animator>();
				oldRoot = animator.GetBoneTransform(HumanBodyBones.Hips);

				if (oldRoot == null)
					oldRoot = go.GetComponentInChildren<SkinnedMeshRenderer>().rootBone;
			...
			for (int i = 0; i < rigids.Length; i++)
			{
				...
				{
					if(rigid.TryGetComponent(out SlicePhysicsIgnorer slicePhysicsIgnorer) == false)
