...
#pragma warning restore 0649
		...
		[SerializeField, NaughtyAttributes.ShowIf(nameof(_changeLayer))] private LayerMask _newLayer;
		[SerializeField] protected ParticleSystem _sparksParticlePrefab;
		[SerializeField] protected ParticleSystem _sliceParticlePrefab;
		[SerializeField, TagField] protected string _unpinRigibodiesAfterSliceTag = "Unpin After Slice";

		[Header("Camera Shake")]
		[SerializeField] protected bool _cameraShakeOnSlice;
		[SerializeField, NaughtyAttributes.ShowIf(nameof(_cameraShakeOnSlice))] protected CameraShakeSettings _cameraShakeSettings;
		[Inject] protected CinemachineVirtualCamera VirtualCamera { set => _noise = value.GetCinemachineComponent<CinemachineBasicMultiChannelPerlin>(); }
		[SerializeField] protected CinemachineBasicMultiChannelPerlin _noise;
		
		[Header("Force After Slice")]
		[SerializeField] protected float _sliceSideForce = 2f;
		[SerializeField] protected float _sliceForwardForce = 10f;
		[SerializeField] protected float _minRotationForce = 3f;
		[SerializeField] protected float _maxRotationForce = 10f;
		...
		public Action OnStartSlice { get; set; }
		public Action OnFinishSlice { get; set; }
		...
		private void StartSlice(BzSliceTryData sliceTryData, Action<BzSliceTryResult> callBack)
		{
			...
			for (int i = 0; i < renderers.Length; i++)
			{
                ...
                if (renderer.TryGetComponent(out SliceRendererIgnorer sliceIgnorer) == true)
                {
                    items[i] = null;
                    continue;
                }
        ...
        protected virtual void GetNewObjects(out GameObject resultObjNeg, out GameObject resultObjPos)
        {
			...
			DestroyCopyWhenSliced[] destroyables = resultObjPos.GetComponentsInChildren<DestroyCopyWhenSliced>();

			foreach (DestroyCopyWhenSliced destroyable in destroyables)
				Destroy(destroyable.gameObject);

			if (_changeLayer == true)
            {
				LayerChanger.ChangeRecursively(resultObjNeg.transform, _newLayer);
				LayerChanger.ChangeRecursively(resultObjPos.transform, _newLayer);
            }
        }
