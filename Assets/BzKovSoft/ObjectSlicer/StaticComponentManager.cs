...
namespace BzKovSoft.ObjectSlicer
{
	class StaticComponentManager : IComponentManager
	{
		...
		{
			List<ColliderSliceResult> results = new List<ColliderSliceResult>(colliders.Length);
			...
			for (int i = 0; i < colliders.Length; i++)
			{
				...
				if (collider.TryGetComponent(out SlicePhysicsIgnorer sliceIgnorer) == true)
					continue;
			...
			return sliced ? results.ToArray() : null;
		}
