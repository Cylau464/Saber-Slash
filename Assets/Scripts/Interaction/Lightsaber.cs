...
public class Lightsaber : MonoBehaviour {
    ...
    [SerializeField] private float _lightIntensityMultiplier = .1f;
    ...
    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioClip soundOn;
    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioClip soundOff;
    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioClip soundLoop;
    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioClip soundSwing;

    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioSource AudioSource;
    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioSource AudioSourceLoop;
    [NaughtyAttributes.ShowIf(nameof(_enableSound))] public AudioSource AudioSourceSwing;
    ...
    private class Blade
    {
        ...
        public void SetColor( Color color)
        {
            ...
            //gameObject.GetComponent<MeshRenderer>().materials[0].SetColor("_Color", color);

        }

        public void updateLight(float intensityMultiplier)
        {
            if (this.light == null)
                return;

            // light intensity depending on blade size
            this.light.intensity = this.scaleCurrent * intensityMultiplier;
        }
    ...
    void InitializeAudio()
    {
        ...
        // initialize audio depending on beam activitiy
        if (saberActive && _enableSound == true)
        {
    ...
    public void Swing(Vector2 delta)
    {
        swingSpeed = delta.magnitude * Time.unscaledDeltaTime;
    }
    ...
    private void updateSwingHandler()
    {
        ...
        if (saberActive && _enableSound == true) // TODO: consider scale, i. e bladeScaleCurrent == bladeScaleMax)
        {
    ...
    private void UpdateBlades()
    {
        foreach (Blade blade in blades)
        {
            ...
            blade.updateLight(_lightIntensityMultiplier);
            blade.updateSize();
